...
import { isFavorite, saveFavortes, removeItem } from "../../utils/storage";
...
export function Detail() {
  ...
  useLayoutEffect(() => {
    async function getStatusFavororite() {
  ...
  async function handleFavoritesReceipe(receipe) {
    if (favorite) {
      ...
      await saveFavortes("@appreceipes", receipe);
      setFavorite(true);
    }
  }
  ...
  async function shareReceipe() {
    try {
      await Share.share({
        ...
        message: `Receitas: ${route.params?.data.name}\nIngredientes: ${route.params?.data.total_ingredients}`,
      });
    } catch (error) {
      console.log(error);
    }
    ...
    >
      <Pressable onPress={HandleOpenVideo}>
        <View style={styles.playIcon}>
          <AntDesign name="playcircleo" color={"#FAFAFA"} size={48} />
        </View>
        <Image
